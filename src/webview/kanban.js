    ...
    if (hasActiveFilters && filteredTasks.length === 0) {
      ...
      if (!document.getElementById("no-results-msg")) {
        ...
        if (filters.search) filterDesc = ` matching "<strong>${filters.search}</strong>"`;
          ...
          filterDesc = ` in the selected ${filters.category && filters.priority ? "category and priority" : filters.category ? "category" : "priority"}`;
  ...
  function getFilteredTasks(tasks) {
    return tasks.filter((task) => {
      const matchesSearch =
        ...
        task.steps?.some((step) => step.toLowerCase().includes(filters.search));
    ...
    modalBody.innerHTML = `
            ...
            ${
              task.dependencies && task.dependencies.length > 0
                ...
                <h3>Dependencies</h3>
                <ul>
                    ${task.dependencies.map((dep) => `<li>${escapeHtml(dep)}</li>`).join("")}
                </ul>
            `
                : ""
            }
