...
export function activate(context: vscode.ExtensionContext) {
	...
	context.subscriptions.push(
		vscode.commands.registerCommand('ralphban.createKanbanBoard', async () => {
			...
			const defaultUri = vscode.Uri.file(path.join(workspaceFolders[0].uri.fsPath, 'prd.json'));
			...
			if (fileUri) {
				...
				vscode.window.showInformationMessage(`Created new Kanban board: ${path.basename(fileUri.fsPath)}`);
			}
		})
	);
